...
public class button_setter{
	...
	public button_setter(handler hand)
	{
		...
		try{
			...
			fire_img = ImageIO.read(new File("Fire_button1.png"));
			fire_img2 = ImageIO.read(new File("Fire_button2.png"));
			
			angle_img = ImageIO.read(new File("red_button04.png"));
			angle_img2 = ImageIO.read(new File("red_button05.png"));
			
			
			
			inc_img = ImageIO.read(new File("inc_img.png"));
			dnc_img  = ImageIO.read(new File("dnc_img.png"));
			
			 f = Font.createFont(Font.TRUETYPE_FONT, new FileInputStream(new File("prototype.ttf"))).deriveFont(Font.PLAIN,15);
	...
	public void render(Graphics G,Player_1 p1,Player_2 p2)//,int variable)//Tank T)			<----- DUMB THING!!!
	{
		...
		//{
			...
			if(p1 != null)
			g.drawString(""+(int)(-hand.get_p1().t.getAngle()),B.angle_x+6,B.angle_y+15+3);
		else
			g.drawString(""+(int)(-hand.get_p2().t.getAngle()),B.angle_x+6,B.angle_y+15+3);
