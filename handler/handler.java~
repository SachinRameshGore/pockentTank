...
public class handler{
	...
	public TURN turn=TUEN.PLAYER1;

	public TURN pop;

	public StartMenu start_menu=new StartMenu();
	public Ending End;

	//public damage_report dr=new damage_report();
	//public damage_terrain dt=new damage_terrain();

	button_specification B;

	LinkedList <shots> shot=new LinkedList <shots>();
	...
	public void render(Graphics G)//**** we need to pass three things Gaphics,STATE,TURN;*******
	{
		if(state==START.GAME|| state==STATE.END)
		{
			terra.render(G);
			p1.render(G);
			p2.render(G);
	...
	public void tick_x(double delta)throws NoninvertibleTransformException // 2things STATE,TURN
	{
		//double dist;
